test_that("index_registry returns valid data.frame with expected columns", {
  ...
  expect_true(nrow(reg) >= 9)

  expected_cols <- c("index", "name", "category", "required_bands", "formula", "range", "reference")
  expect_true(all(expected_cols %in% names(reg)))

  expected_indices <- c("NDVI", "SAVI", "EVI", "GNDVI", "NDWI", "MNDWI", "NDBI", "NDMI", "BSI")
...
test_that("index_registry filters by category", {
  ...
  expect_warning(index_registry(category = "nonexistent_category"), "Unknown category")
})
...
test_that("get_index_meta returns correct metadata and handles unknown index", {
  ...
  expect_error(get_index_meta("INVALID_INDEX"), "Unknown index")
})
